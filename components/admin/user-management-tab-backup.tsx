    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (onStatsUpdate && response.data) {
          const activeUsers = response.data.items?.filter((u: UserResponse) => u.is_active).length || 0
          const pendingUsers = response.data.items?.filter((u: UserResponse) => !u.is_active).length || 0
