        ...
        refreshJobStatus: async (jobId: string) => {
          try {
            ...
            if (jobStatus.status === 'completed' && typeof window !== 'undefined') {
              console.log(`🎯 Bulk upload job completed successfully, refreshing predictions and stats for ${jobId}`)

              try {
                // Ensure latest predictions are fetched
                const predictionsStore = (await import('@/lib/stores/predictions-store')).usePredictionsStore.getState()
                await predictionsStore.refetchPredictions()
              } catch (e) {
                console.warn('Failed to refetch predictions after bulk job completion:', e)
              }

              try {
                // Refresh dashboard statistics
                const statsStore = (await import('@/lib/stores/dashboard-stats-store')).useDashboardStatsStore.getState()
                statsStore.invalidateCache()
                await statsStore.fetchStats(true)
              } catch (e) {
                console.warn('Failed to refresh dashboard stats after bulk job completion:', e)
              }

              // Now notify the app that data is updated
